...
fert_trends_c1 <- ggplot(data_long, aes(x = Year, y = Emissions, color = Country_abbr, group = Country)) +
  geom_line(size = 0.8) +
  geom_point(size = 2) +
  ...
  theme_minimal() +
  labs(
    # title = "Tasa de fertilidad por país",
    subtitle = "2010–2014",
    x = "Año",
...
fert_trends_c2 <- ggplot(data_long_2, aes(x = Year, y = Tasa_de_fertilidad, color = Country_abbr, group = Country)) +
  geom_line(size = 0.8) +
  geom_point(size = 2) +
...
fert_trends_c3 <- ggplot(data_long_3, aes(x = Year, y = Tasa_de_fertilidad, color = Country_abbr, group = Country)) +
  geom_line(size = 0.8) +
  geom_point(size = 2) +
  facet_wrap(~Cluster, ncol = 1, strip.position = "left", scales = "free_y") +
...
# Save as PNG
png("tft_evolution.png", width=2000, height=1000, res=400)

cp <- fert_trends_c1 | fert_trends_c2 | fert_trends_c3

ggsave("tft_evolution.png", cp, width=12, height=4, dpi=300)
```
...
# 1. Preparar años y escalar datos
anios_totales <- as.character(2010:2023)
data_todo_periodo <- data_original[, c("Country", anios_totales)]

data_scaled <- scale(data_todo_periodo[, -1])
hclust_result <- hclust(dist(data_scaled))
clusters <- cutree(hclust_result, k = 3)

# 2. Agregar cluster
data_clustered <- data_todo_periodo %>%
  mutate(
...
data_long <- data_clustered %>%
  pivot_longer(cols = all_of(anios_totales), names_to = "Year", values_to = "Tasa") %>%
  mutate(
    Year = as.numeric(Year),
    Country_abbr = substr(Country, 1, 3)
  )

# 4. Crear paleta de colores igual a la usada por ggplot por defecto
# Esto reproduce el orden de colores por país
paises_ordenados <- sort(unique(data_long$Country_abbr))
colores_default <- scale_color_discrete()$palette(length(paises_ordenados))
colores_fijos <- setNames(colores_default, paises_ordenados)

# 5. Años espaciados en eje X
breaks_eje_x <- seq(2010, 2023, by = 2)

# 6. Graficar por cluster con colores iguales al gráfico original 2020–2023
for (cl in levels(data_long$Cluster)) {
  data_cluster_i <- data_long %>% filter(Cluster == cl)
  
  p <- ggplot(data_cluster_i, aes(x = Year, y = Tasa, color = Country_abbr, group = Country)) +
    geom_line(size = 0.8) +
    geom_point(size = 2) +
    scale_x_continuous(breaks = breaks_eje_x) +
    scale_color_manual(values = colores_fijos) +
    theme_minimal(base_size = 14) +
    labs(
      title = paste("Tasa de Fertilidad (2010–2023) – Cluster", cl),
      x = "Año", y = "Tasa estandarizada", color = "País"
    ) +
    theme(
      plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
      axis.text.x = element_text(size = 8),
  ...
  # Guardar si deseas
  ggsave(paste0("cluster_", cl, "_fertilidad_colores2020.png"), p, width = 12, height = 5, dpi = 300)
}
```
...
# 2. Leer datos de fertilidad
fertility_data <- read.csv("tabla_resumen_fertilidad_transpuesta.csv", check.names = FALSE)
colnames(fertility_data)[1] <- "Country"

# Convertir fertilidad a formato largo
fertility_long <- fertility_data %>%
  pivot_longer(-Country, names_to = "Year", values_to = "FertilityRate") %>%
  mutate(Year = as.numeric(Year))

# Verifica si Country coincide con geoUnit — si no, arreglamos con ISO3
# Por ahora creamos abreviatura ficticia de 3 letras
fertility_long <- fertility_long %>%
  mutate(geoUnit = substr(Country, 1, 3))

# Unión de datos
merged_compare <- fertility_long %>%
  left_join(education_completion_rates, by = c("geoUnit", "Year" = "year")) %>%
  drop_na(education_completion_rate, FertilityRate)

# ⚠️ Verifica que hayan datos
if (nrow(merged_compare) == 0) {
  stop("No se encontraron coincidencias entre fertilidad y educación. Verifica los códigos de país y los años.")
}

# Escalar por país
merged_compare_scaled <- merged_compare %>%
  group_by(Country) %>%
  mutate(
    fertility_scaled = scale(FertilityRate),
    education_scaled = scale(education_completion_rate)
  ) %>%
  ungroup()

# Gráfico comparativo (corrigiendo el uso de linewidth)
ggplot(merged_compare_scaled, aes(x = Year)) +
  geom_line(aes(y = fertility_scaled, color = "Tasa de Fertilidad"), linewidth = 1) +
  geom_line(aes(y = education_scaled, color = "Finalización Educativa"), linewidth = 1, linetype = "dashed") +
  facet_wrap(~Country, scales = "free_y") +
  labs(
    title = "Comparación entre Tasa de Fertilidad y Finalización de Educación",
    subtitle = "Valores estandarizados por país",
...
# Revisa los países y años
unique(fertility_data$Country)
...
unique(education_completion_rates$geoUnit)
unique(education_completion_rates$year)
```
```{r}
library(tidyverse)
library(readxl)
...
# Crear mapeo manual de país a ISO3
iso_mapping <- tibble(
  Country = c("Argentina", "Bolivia", "Brazil", "Chile", "Colombia", "Ecuador",
              "Paraguay", "Peru", "Uruguay", "Venezuela"),
  geoUnit = c("ARG", "BOL", "BRA", "CHL", "COL", "ECU", "PRY", "PER", "URY", "VEN")
)

# Unir para obtener código ISO3
fertility_long <- fertility_long %>%
...
# Confirmar unión
print(nrow(merged_compare))
head(merged_compare)

```

```{r}
ggplot(merged_compare, aes(x = education_completion_rate, y = FertilityRate, color = Country)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  facet_wrap(~Year) +
  theme_minimal() +
  labs(
    title = "Relación entre tasa de finalización educativa y fertilidad",
    x = "Tasa de finalización educativa (%)",

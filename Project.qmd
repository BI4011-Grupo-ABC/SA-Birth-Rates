...
data_poisson <- merged_data %>%
  select(Country, Year, Births_le18, education_completion_rate.x, TotalWomen_le18, hdi, gdp_per_capita) %>%
  na.omit()
...
# Modelo de Poisson
modelo_poisson <- glm(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp_per_capita,
                      data = data_poisson,
...
```{r}
modelo_quasi <- glm(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp_per_capita,
                    data = data_poisson,
...
modelo_nb <- glm.nb(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp_per_capita,
                    data = data_poisson)
...
```{r}
poisson_model <- glm(
  Births_le18 ~  education_completion_rate.x + hdi + gdp_per_capita + Year,
  data = merged_data,
  family = poisson(link = "log"),
...
irr_data$Variable <- factor(irr_data$Variable,
                             levels = c("Year", "gdp_per_capita", "hdi", "education_completion_rate.x", "(Intercept)"))
...
```{r}
library(ggplot2)
ggplot(merged_data, aes(x = hdi, y = Births_le18 / TotalWomen_le18)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "glm.nb", formula = y ~ x, se = FALSE, color = "blue") + # O method = "lm" si quieres una línea recta
  labs(title = "Tasa de Fertilidad Adolescente vs. HDI",
       x = "HDI",

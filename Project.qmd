...
merged_data <- merged_data %>%
  select(Country, `ISO3 Alpha-code`, Year, `FertilityRate_le18`, Value)
```

```{r}
m_all <- lm(merged_data$FertilityRate_le18 ~ merged_data$Value)
summary(m_all)
```
...
merged_data_peru <- merged_data %>%
  filter(Country == "Peru")
m_peru <- lm(merged_data_peru$FertilityRate_le18 ~ merged_data_peru$Value)
summary(m_peru)
```
...
country_models <- merged_data %>%
  ...
  mutate(
    ...
    model = map(data, ~lm(FertilityRate_le18 ~ Value, data = .x)),
    # Extract results
...
m_all <- lm(FertilityRate_le18 ~ Value, data = merged_data)
summary(m_all)
...
Linealidad1 <- ggplot(data = merged_data, aes(Value, resid(m_all))) +
  geom_point() +
...
m_all2 <- lm(FertilityRate_le18 ~ Value + I(Value^2), data = merged_data)
summary(m_all2)

# Volver a graficar residuos con el modelo cuadrático
Linealidad1_poly <- ggplot(data = merged_data, aes(Value, resid(m_all2))) +
  geom_point() +
...
library(mgcv)
m_gam <- gam(FertilityRate_le18 ~ s(Value) + s(Year), data = merged_data)
summary(m_gam)

# Gráfico de residuos para el GAM
Linealidad1_gam <- ggplot(data = merged_data, aes(Value, resid(m_gam))) +
  geom_point() +

...
rownames(data_range_) <- substr(tabla_resumen$Country, 1, 3)
...
ggplot(data_long_2, aes(x = Year, y = `Tasa de fertilidad`, color = Country, group = Country)) +
  ...
  theme(
    ...
    legend.title = element_text(face = "bold", size = 9),
    legend.position = "right",
    panel.spacing = unit(1.5, "lines")                 # Aumentar espacio entre clusters
  )
...
pisa_scores_interpolated <- pisa_scores %>%
  group_by(CIMA_Indicator, Country, Source, Subject, Grade, Class) %>%
  complete(Year = full_seq(Year, 1)) %>%
  arrange(Year) %>%
  mutate(Value = approx(Year, Value, Year, rule=2)$y) %>%
  ungroup()

pisa_scores_interpolated_wide <- pisa_scores_interpolated %>%
  pivot_wider(names_from = Year, values_from = Value)

pisa_2010_2022 <- pisa_scores_interpolated_wide %>%
  select(CIMA_Indicator, Country, Source, Subject, Grade, Class, `2010`:`2022`)

pisa_2010_2022_science <- pisa_2010_2022 %>% 
...
year_cols <- as.character(start_year:end_year)
pisa_2010_2014 <- pisa_2010_2022_science[, year_cols]
pisa_2010_2014_scaled <- scale(pisa_2010_2014)
```

```{r}
elbow(pisa_2010_2014_scaled)
clusterize(pisa_2010_2014_scaled, k=3)
```

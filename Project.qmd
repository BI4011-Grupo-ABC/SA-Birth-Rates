...
# Función para graficar mapas de fertilidad con leyenda por rangos
draw_heat_map_fertility <- function() {
  message("Generando mapa de Fertilidad...")

  # Hacer la unión usando nombre de país
  map_data <- south_america %>%
  ...
  # Crear categorías/rangos personalizados
  breaks <- seq(floor(min(map_data$FertilityRate_le18, na.rm = TRUE)),
                ceiling(max(map_data$FertilityRate_le18, na.rm = TRUE)),
                length.out = 6)  # Puedes ajustar el número de rangos

  map_data <- map_data %>%
    mutate(FertilityRateRange = cut(FertilityRate_le18, breaks = breaks, include.lowest = TRUE))

  # Crear el mapa con leyenda categórica
  ggplot(data = map_data) +
...
hdi_countries <- hdi_countries %>%
  ...
  dplyr::select(countryIsoCode, year, value) %>%
  mutate(year = as.double(year)) %>%
  rename(hdi = value) %>%
  mutate(hdi = as.numeric(hdi))

merged_data <- merged_data %>%
  left_join(hdi_countries,
...
data_poisson <- merged_data %>%
  select(Country, Year, Births_le18, education_completion_rate.x, TotalWomen_le18, hdi, gdp) %>%
  na.omit()
...
# Modelo de Poisson
modelo_poisson <- glm(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp,
                      data = data_poisson,
...
```{r}
modelo_quasi <- glm(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp,
                    data = data_poisson,
...
modelo_nb <- glm.nb(Births_le18 ~ education_completion_rate.x + TotalWomen_le18 + hdi + gdp,
                    data = data_poisson)
...
```{r}
poisson_model <- glm(
  Births_le18 ~  education_completion_rate.x + hdi + gdp + Year,
  data = merged_data,
  family = poisson(link = "log"),
...
Modelo_BN <- glm.nb(
  Births_le18 ~ education_completion_rate.x + hdi + gdp + Year + offset(log(TotalWomen_le18)),
  data = merged_data
)

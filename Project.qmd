...
n_obs <- nrow(data_scaled_1)  # Number of rows in the scaled data

# Plot the elbow method with a limit on the number of clusters
fviz_nbclust(data_scaled_1, kmeans, method = "wss", k.max = min(10, n_obs - 1)) +
...
distance_matrix_1 <- dist(data_scaled_1, method = "euclidean")

# Perform hierarchical clustering (using complete linkage method)
hclust_result_1 <- hclust(distance_matrix_1, method = "complete")
...
fviz_dend(hclust_result, k = 3,  # Number of desired clusters
          ...
          rect_border = "gray",
          rect_fill = TRUE,
          main = "Dendrogram of Hierarchical Clustering (2010 - 2014)")
```

```{r}
# Assign observations to clusters (k = desired number of clusters)
cluster_assignments_1 <- cutree(hclust_result_1, k = 3)  # Adjust "k" for the desired number of clusters

# Visualize clusters in PCA space
fviz_cluster(
  list(data = data_scaled_1, cluster = cluster_assignments_1),
  geom = "point",
...
```{r}
n_obs_2 <- nrow(data_scaled_2)  # Number of rows in the scaled data

# Plot the elbow method with a limit on the number of clusters
fviz_nbclust(data_scaled_2, kmeans, method = "wss", k.max = min(10, n_obs - 1)) +
  labs(title = "Elbow Method to Determine the Number of Clusters (2010 - 2014)")
```
...
# Calculate the distance matrix (using Euclidean distance)
distance_matrix_2 <- dist(data_scaled_2, method = "euclidean")

# Perform hierarchical clustering (using complete linkage method)
hclust_result_2 <- hclust(distance_matrix_2, method = "complete")
...
```{r}
# Assign observations to clusters (k = desired number of clusters)
cluster_assignments_2 <- cutree(hclust_result_2, k = 3)  # Adjust "k" for the desired number of clusters

# Visualize clusters in PCA space
fviz_cluster(
  list(data = data_scaled_2, cluster = cluster_assignments_2),
  geom = "point",
  ellipse.type = "convex",  # Draw convex hulls around clusters
  ggtheme = theme_minimal(),
```
...
```{r}
n_obs_3 <- nrow(data_scaled_3)  # Number of rows in the scaled data

# Plot the elbow method with a limit on the number of clusters
fviz_nbclust(data_scaled_3, kmeans, method = "wss", k.max = min(10, n_obs - 1)) +
  labs(title = "Elbow Method to Determine the Number of Clusters (2010 - 2014)")
```
...
# Calculate the distance matrix (using Euclidean distance)
distance_matrix_3 <- dist(data_scaled_3, method = "euclidean")

# Perform hierarchical clustering (using complete linkage method)
hclust_result_3 <- hclust(distance_matrix_3, method = "complete")
...
```{r}
# Assign observations to clusters (k = desired number of clusters)
cluster_assignments_3 <- cutree(hclust_result_2, k = 3)  # Adjust "k" for the desired number of clusters

# Visualize clusters in PCA space
fviz_cluster(
  list(data = data_scaled_3, cluster = cluster_assignments_3),
  geom = "point",
  ellipse.type = "convex",  # Draw convex hulls around clusters
  ggtheme = theme_minimal(),
  main = "Hierarchical Clustering Visualization with PCA"

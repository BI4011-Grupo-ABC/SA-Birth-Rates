...
```{r}
summary(m1)
m_all <- lm(merged_data$Value ~ merged_data$FertilityRate_le18)
```
```{r}
merged_data_peru <- merged_data %>%
  filter(Country == "Peru")
m_peru <- lm(merged_data_peru$Value ~ merged_data_peru$FertilityRate_le18)
summary(m_peru)
```
...
country_models <- merged_data %>%
  ...
  mutate(
    ...
    model = map(data, ~lm(Value ~ FertilityRate_le18, data = .x)),
    # Extract results
